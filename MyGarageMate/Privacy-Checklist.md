...
Version 1 does not send MyGarageMate car maintenance data to a backend server. Sign in with Apple is used only for local account identity.
...
- Notifications let MyGarageMate remind users about oil changes, insurance, inspections, tires, and custom maintenance tasks.
...
- Location: MyGarageMate does not request location access.
- Contacts, calendar, microphone, camera: MyGarageMate does not request these permissions in version 1.
